...
func randomClub(from []structs.Club) structs.Club {
	randomness := rand.New(rand.NewSource(int64(time.Now().Nanosecond())))
	return from[randomness.Intn(len(from))]
}
